...
function parseMonsterActions(description) {
  ...
  const parsedActions = rawActions.map(action => {
      ...
      // Attack
      const attackMatch = action.fullText.match(/(?:Melee Weapon Attack:|Ranged Weapon Attack:|Weapon Attack:)\s*([+-]?\d+)\s*to\s*hit/i);
      if (attackMatch) {
      ...
      // Damage: FIRST look for ANY dice notation without parentheses (your screenshot format!)
      const anyDiceRegex = /(\d+d\d+(?:\s*[+-]\s*\d+)?)/gi;
      let anyDiceMatch;
      while ((anyDiceMatch = anyDiceRegex.exec(action.fullText)) !== null) {
      ...
      // Then also check for parenthesized dice notation (standard format) as backup
      ...
      // Remove duplicates (same dice expression)
      const seenDice = new Set();
      details.damages = details.damages.filter(damage => {
          const normalized = damage.dice.replace(/\s+/g, '');
          if (seenDice.has(normalized)) return false;
          seenDice.add(normalized);
          return true;
      });
...
function getOverrideList(type) {
    if (type === 'monster') return getOverrideMonsters();
    if (type === 'item') return getOverrideItems();
    return getOverrideSpells();
}

function isOverrideEntry(name, type) {
...
function findBuiltInOrigin(type, originName) {
    ...
    if (type === 'monster') return findBuiltInMonster(originName);
    if (type === 'item') return findBuiltInItem(originName);
    return findBuiltInSpell(originName);
}
...
function getOverrideMonsters() {
    try { return JSON.parse(localStorage.getItem('dnd_extension_override_monsters') || '[]'); } catch (e) { return []; }
}
function getOverrideItems() {
    try { return JSON.parse(localStorage.getItem('dnd_extension_override_items') || '[]'); } catch (e) { return []; }
}
function getOverrideSpells() {
    try { return JSON.parse(localStorage.getItem('dnd_extension_override_spells') || '[]'); } catch (e) { return []; }
}
...
function preserveSourceBookMetadata(entry, type, originName = null) {
    return applySourceBookOrigin(entry, type, originName || entry.name);
}

function removeFromEntryList(type, name, listKind) {
    const storageKeys = {
        monster: { override: 'dnd_extension_override_monsters', custom: 'dnd_extension_custom_monsters' },
        item: { override: 'dnd_extension_override_items', custom: 'dnd_extension_custom_items' },
        spell: { override: 'dnd_extension_override_spells', custom: 'dnd_extension_custom_spells' },
    };
    const getters = {
        override: getOverrideList,
        custom: (t) => t === 'monster' ? getCustomMonsters() : t === 'item' ? getCustomItems() : getCustomSpells(),
    };
    const key = storageKeys[type][listKind];
    const list = (listKind === 'override' ? getOverrideList(type) : getters.custom(type))
        .filter((entry) => !namesMatch(entry.name, name));
    localStorage.setItem(key, JSON.stringify(list));
}

function migrateEntryAssets(type, oldName, newName) {
    if (!oldName || !newName || namesMatch(oldName, newName)) return;
    const oldImgKey = getEntryImageKey(type, oldName);
    const newImgKey = getEntryImageKey(type, newName);
    const img = localStorage.getItem(oldImgKey);
    if (img) {
        localStorage.setItem(newImgKey, img);
        localStorage.removeItem(oldImgKey);
    }
    const oldHistoryKey = `dnd_extension_entry_images_${oldName}`;
    const newHistoryKey = `dnd_extension_entry_images_${newName}`;
...
function hideSourceBookOriginal(type, originName) {
    if (!originName || !isSourceBookEntry(originName, type)) return false;
    const builtIn = findBuiltInOrigin(type, originName);
    const canonical = builtIn?.name || normalizeEntryName(originName);
    const list = getDeletedItems();
    if (list.some((d) => namesMatch(d, canonical))) return false;
    list.push(canonical);
    localStorage.setItem('dnd_extension_deleted_items', JSON.stringify(list));
    return true;
}

function ensureOverrideHidesBuiltIn(type, entry, originName = null) {
    let hidden = false;
    const origins = new Set();
    if (entry?.originBookName) origins.add(entry.originBookName);
    if (originName) origins.add(originName);
    for (const origin of origins) {
        if (hideSourceBookOriginal(type, origin)) hidden = true;
    }
    return hidden;
}

function handleSourceBookRename(type, oldName, newName) {
    if (!oldName || !newName || namesMatch(oldName, newName)) return;
    hideSourceBookOriginal(type, oldName);
    removeFromEntryList(type, oldName, 'override');
    removeFromEntryList(type, oldName, 'custom');
    migrateEntryAssets(type, oldName, newName);
}

function getBuiltInList(type) {
    if (type === 'monster') return monsters;
    if (type === 'item') return items;
    return Object.entries(SPELL_DATA).map(([name, data]) => ({ name, ...data }));
}

function guessBuiltInOriginForOverride(override, type) {
    if (override.originBookName) return override.originBookName;
    const builtIns = getBuiltInList(type);
    const deleted = getDeletedItems();
    const overrideName = normalizeEntryName(override.name).toLowerCase().replace(/[,.\s]+$/g, '').trim();
    if (!override.source) return null;

    const candidates = builtIns.filter((builtIn) => {
        if (builtIn.source !== override.source) return false;
        if (namesMatch(builtIn.name, override.name)) return false;
        if (deleted.some((d) => namesMatch(d, builtIn.name))) return false;
        const typeHint = normalizeEntryName(builtIn.type || '').toLowerCase();
        if (!typeHint) return false;
        const primaryName = typeHint.split('*')[0].trim();
        if (primaryName.length < 4) return false;
        return overrideName.includes(primaryName) || primaryName.includes(overrideName);
    });

    return candidates.length === 1 ? candidates[0].name : null;
}

function repairRenamedSourceBookOverrides({ save = true } = {}) {
    let changed = false;
    const storageKeys = {
        monster: 'dnd_extension_override_monsters',
        item: 'dnd_extension_override_items',
        spell: 'dnd_extension_override_spells',
    };

    for (const type of ['monster', 'item', 'spell']) {
        const list = getOverrideList(type);
        let listChanged = false;
        for (let i = 0; i < list.length; i++) {
            const entry = list[i];
            const guessed = entry.originBookName || guessBuiltInOriginForOverride(entry, type);
            if (!guessed) continue;
            const normalized = preserveSourceBookMetadata(
                { ...entry, originBookName: normalizeEntryName(guessed) },
                type,
                guessed
            );
            if (JSON.stringify(normalized) !== JSON.stringify(list[i])) {
                list[i] = normalized;
                listChanged = true;
            }
            if (hideSourceBookOriginal(type, guessed)) changed = true;
        }
        if (listChanged) {
    ...
    if (changed && save) saveToBackend();
    return changed;
}

function getCustomList(type) {
    if (type === 'monster') return getCustomMonsters();
    if (type === 'item') return getCustomItems();
    return getCustomSpells();
}

function dedupeOverridesByName(overrides) {
    const map = new Map();
    for (const entry of overrides || []) {
        if (!entry?.name) continue;
        map.set(normalizeEntryName(entry.name).toLowerCase(), entry);
    }
    return Array.from(map.values());
}

function stripReplacedFromCustom(customs, overrides, type) {
    const overrideNames = new Set(
        overrides.map((entry) => normalizeEntryName(entry.name).toLowerCase())
    );
    const replacedOrigins = new Set(
        overrides
            .filter((entry) => entry.originBookName)
            .map((entry) => normalizeEntryName(entry.originBookName).toLowerCase())
    );
    return (customs || []).filter((entry) => {
        const name = normalizeEntryName(entry.name).toLowerCase();
        if (overrideNames.has(name)) return false;
        if (replacedOrigins.has(name)) return false;
        if (entry.originBookName) return false;
        if (isSourceBookEntry(entry.name, type)) return false;
        return true;
    });
}

function buildDeletedForMongo(overrides, deleted) {
    const nextDeleted = [...(deleted || [])];
    const add = (name) => {
        if (!name) return;
        if (!nextDeleted.some((entry) => namesMatch(entry, name))) {
            nextDeleted.push(normalizeEntryName(name));
        }
    };
    for (const override of overrides) {
        if (override.originBookName && !namesMatch(override.originBookName, override.name)) {
            add(override.originBookName);
        }
    }
    return nextDeleted;
}

function normalizeMongoSnapshot({ persist = true } = {}) {
    reconcileCustomAndOverrides({ save: false });
    repairRenamedSourceBookOverrides({ save: false });

    const storage = {
        monster: {
            customKey: 'dnd_extension_custom_monsters',
            overrideKey: 'dnd_extension_override_monsters',
        },
        item: {
            customKey: 'dnd_extension_custom_items',
            overrideKey: 'dnd_extension_override_items',
        },
        spell: {
            customKey: 'dnd_extension_custom_spells',
            overrideKey: 'dnd_extension_override_spells',
        },
    };

    let deleted = getDeletedItems();
    for (const type of ['monster', 'item', 'spell']) {
        const overrides = dedupeOverridesByName(getOverrideList(type));
        const customs = stripReplacedFromCustom(getCustomList(type), overrides, type);
        deleted = buildDeletedForMongo(overrides, deleted);
        for (const override of overrides) {
            if (override.originBookName) hideSourceBookOriginal(type, override.originBookName);
        }
        if (persist) {
            localStorage.setItem(storage[type].overrideKey, JSON.stringify(overrides));
            localStorage.setItem(storage[type].customKey, JSON.stringify(customs));
        }
    }

    deleted = getDeletedItems();
    if (persist) {
        localStorage.setItem('dnd_extension_deleted_items', JSON.stringify(deleted));
    }

    return {
        monsters: getCustomMonsters(),
        items: getCustomItems(),
        spells: getCustomSpells(),
        overrideMonsters: getOverrideMonsters(),
        overrideItems: getOverrideItems(),
        overrideSpells: getOverrideSpells(),
        deleted: getDeletedItems(),
    };
}
...
function saveOverrideMonster(monster, { originName = null } = {}) {
    const list = getOverrideMonsters();
    const entry = preserveSourceBookMetadata(attachImageToEntry(monster, 'monster'), 'monster', originName);
    const index = list.findIndex((m) => namesMatch(m.name, entry.name));
    if (index >= 0) list[index] = entry;
    else list.push(entry);
    localStorage.setItem('dnd_extension_override_monsters', JSON.stringify(list));
    const customs = getCustomMonsters().filter((m) =>
        !namesMatch(m.name, entry.name) &&
        !(entry.originBookName && namesMatch(m.name, entry.originBookName)) &&
        !(originName && namesMatch(m.name, originName))
    );
    localStorage.setItem('dnd_extension_custom_monsters', JSON.stringify(customs));
    ensureOverrideHidesBuiltIn('monster', entry, originName);
    saveToBackend();
    reconcileCustomAndOverrides({ save: false });
    repairRenamedSourceBookOverrides({ save: false });
...
function saveOverrideItem(item, { originName = null } = {}) {
    const list = getOverrideItems();
    const entry = preserveSourceBookMetadata(attachImageToEntry(item, 'item'), 'item', originName);
    const index = list.findIndex((i) => namesMatch(i.name, entry.name));
    if (index >= 0) list[index] = entry;
    else list.push(entry);
    localStorage.setItem('dnd_extension_override_items', JSON.stringify(list));
    const customs = getCustomItems().filter((i) =>
        !namesMatch(i.name, entry.name) &&
        !(entry.originBookName && namesMatch(i.name, entry.originBookName)) &&
        !(originName && namesMatch(i.name, originName))
    );
    localStorage.setItem('dnd_extension_custom_items', JSON.stringify(customs));
    ensureOverrideHidesBuiltIn('item', entry, originName);
    saveToBackend();
    reconcileCustomAndOverrides({ save: false });
    repairRenamedSourceBookOverrides({ save: false });
    restoreItem(entry.name);
...
function saveOverrideSpell(spell, { originName = null } = {}) {
    const list = getOverrideSpells();
    const entry = preserveSourceBookMetadata(attachImageToEntry(spell, 'spell'), 'spell', originName);
    const index = list.findIndex((s) => namesMatch(s.name, entry.name));
    if (index >= 0) list[index] = entry;
    else list.push(entry);
    localStorage.setItem('dnd_extension_override_spells', JSON.stringify(list));
    const customs = getCustomSpells().filter((s) =>
        !namesMatch(s.name, entry.name) &&
        !(entry.originBookName && namesMatch(s.name, entry.originBookName)) &&
        !(originName && namesMatch(s.name, originName))
    );
    localStorage.setItem('dnd_extension_custom_spells', JSON.stringify(customs));
    ensureOverrideHidesBuiltIn('spell', entry, originName);
    saveToBackend();
    reconcileCustomAndOverrides({ save: false });
    repairRenamedSourceBookOverrides({ save: false });
    restoreItem(entry.name);
...
function resolveRenameFromName(type, originalName, entryName, bookKey = null) {
    return [originalName, bookKey].find((name) =>
        name &&
        !namesMatch(name, entryName) &&
        (isSourceBookEntry(name, type) || isOverrideEntry(name, type))
    ) || null;
}
...
function saveMonsterEntry(monster, { originalName = null, bookKey = null } = {}) {
    ...
    if (shouldSaveAsOverride('monster', originalName, full.name, key)) {
        const renameFrom = resolveRenameFromName('monster', originalName, full.name, key);
        if (renameFrom) handleSourceBookRename('monster', renameFrom, full.name);
...
function saveItemEntry(item, { originalName = null, bookKey = null } = {}) {
    ...
    if (shouldSaveAsOverride('item', originalName, full.name, key)) {
        const renameFrom = resolveRenameFromName('item', originalName, full.name, key);
        if (renameFrom) handleSourceBookRename('item', renameFrom, full.name);
...
function saveSpellEntry(spell, { originalName = null, bookKey = null } = {}) {
    ...
    if (shouldSaveAsOverride('spell', originalName, full.name, key)) {
        const renameFrom = resolveRenameFromName('spell', originalName, full.name, key);
        if (renameFrom) handleSourceBookRename('spell', renameFrom, full.name);
...
function mergeSourceBookEntries(builtIns, overrides, deleted) {
    const isHiddenBuiltIn = (builtInName) =>
        overrides.some((o) =>
            namesMatch(o.name, builtInName) ||
            (o.originBookName && namesMatch(o.originBookName, builtInName))
        ) ||
        deleted.some((d) => namesMatch(d, builtInName));

    const activeBuiltIns = builtIns.filter((entry) => !isHiddenBuiltIn(entry.name));
    return [...activeBuiltIns, ...overrides].filter(
        (entry) => !deleted.some((d) => namesMatch(d, entry.name))
    );
}

function migrateLegacyOverrides() {
    if (localStorage.getItem(OVERRIDE_MIGRATION_KEY)) return;

    const overrideMonsters = getOverrideMonsters();
    const overrideItems = getOverrideItems();
    const overrideSpells = getOverrideSpells();
    let customMonsters = getCustomMonsters();
    let customItems = getCustomItems();
    let customSpells = getCustomSpells();

    customMonsters.filter(m => isBuiltInMonster(m.name)).forEach(m => {
        const idx = overrideMonsters.findIndex(o => o.name === m.name);
        const entry = preserveSourceBookMetadata(m, 'monster');
        if (idx === -1) overrideMonsters.push(entry);
        else overrideMonsters[idx] = { ...overrideMonsters[idx], ...entry };
    });
    customMonsters = customMonsters.filter(m => !isBuiltInMonster(m.name));

    customItems.filter(i => isBuiltInItem(i.name)).forEach(i => {
        const idx = overrideItems.findIndex(o => o.name === i.name);
        const entry = preserveSourceBookMetadata(i, 'item');
        if (idx === -1) overrideItems.push(entry);
        else overrideItems[idx] = { ...overrideItems[idx], ...entry };
    });
    customItems = customItems.filter(i => !isBuiltInItem(i.name));

    customSpells.filter(s => isBuiltInSpell(s.name)).forEach(s => {
        const idx = overrideSpells.findIndex(o => o.name === s.name);
        const entry = preserveSourceBookMetadata(s, 'spell');
        if (idx === -1) overrideSpells.push(entry);
        else overrideSpells[idx] = { ...overrideSpells[idx], ...entry };
    });
    customSpells = customSpells.filter(s => !isBuiltInSpell(s.name));
    ...
    localStorage.setItem(OVERRIDE_MIGRATION_KEY, '1');
}
...
function reconcileCustomAndOverrides({ save = true } = {}) {
    ...
    const shouldMoveToOverrides = (entry, type) =>
        isSourceBookEntry(entry.name, type) ||
        (entry.originBookName && isSourceBookEntry(entry.originBookName, type));

    const moveMisplaced = (customList, overrideList, type) => {
        const toMove = customList.filter((entry) => shouldMoveToOverrides(entry, type));
        ...
        changed = true;
        return customList.filter((entry) => !shouldMoveToOverrides(entry, type));
    };
...
function restoreItem(name) {
     try {
         ...
         const idx = list.findIndex((d) => namesMatch(d, name));
         if (idx !== -1) {
             list.splice(idx, 1);
             localStorage.setItem('dnd_extension_deleted_items', JSON.stringify(list));
             saveToBackend();
...
function setLastSyncedAt(value) {
    if (!value) return;
    localStorage.setItem(LAST_SYNCED_KEY, new Date(value).toISOString());
}

function isRemoteNewer(remoteLastUpdated) {
    if (!remoteLastUpdated) return true;
    const localLastSynced = getLastSyncedAt();
    if (!localLastSynced) return true;
    return new Date(remoteLastUpdated).getTime() > new Date(localLastSynced).getTime();
}
...
function applyRemoteSyncData(data) {
    ...
    reconcileCustomAndOverrides({ save: false });
    if (repairRenamedSourceBookOverrides({ save: false })) changed = true;
    normalizeMongoSnapshot({ persist: true });
    return changed;
}
...
async function syncWithBackend({ force = false } = {}) {
    ...
    try {
        ...
        if (!force) {
            const versionRes = await fetch(`${API_BASE}/data/version`);
            if (versionRes.ok) {
                const version = await versionRes.json();
                if (!isRemoteNewer(version.lastUpdated)) {
                    return false;
                }
            }
        }
...
function startRealtimeSync() {
    ...
    realtimePollTimer = setInterval(async () => {
        try {
            ...
            const version = await versionRes.json();
            await handleRemoteDataUpdate(version.lastUpdated);
        } catch (e) {
...
function collectBackendPayload() {
    const snapshot = normalizeMongoSnapshot({ persist: true });
    const monsters = snapshot.monsters;
    const items = snapshot.items;
    const spells = snapshot.spells;
    const overrideMonsters = snapshot.overrideMonsters;
    const overrideItems = snapshot.overrideItems;
    const overrideSpells = snapshot.overrideSpells;
...
export function searchSpells(query) {
  const deleted = getDeletedItems();
  const overrides = getOverrideSpells();
  const builtInSpells = Object.entries(SPELL_DATA).map(([name, data]) => ({ name, ...data }));
  const allSpells = mergeSourceBookEntries(builtInSpells, overrides, deleted);
...
export function searchMonsters(query, searchNameOnly = false, minCrStr = '', maxCrStr = '') {
  const deleted = getDeletedItems();
  const overrides = getOverrideMonsters();
  const allMonsters = mergeSourceBookEntries(monsters, overrides, deleted);
...
export async function addMonsterToScene(monster) {
  ...
  export async function addItemToScene(itemData) {
      ...
      if (imageUrl && (typeof imageUrl !== 'string' || imageUrl.trim().startsWith('{') || imageUrl.trim().startsWith('%7B'))) {
       ...
       if (!imageUrl || (isPlaceholder && imageUrl.includes('Tm90IEZvdW5k'))) {
...
async function processAndRemoveBackground(source) {
  // PhotoRoom disabled for now (API key issues), using reliable local method!
  console.log('Using local background removal method...');
  
  // Use local background removal method
  return new Promise((resolve, reject) => {
...
function deleteItem(name) {
    if (hideSourceBookOriginal('monster', name) || hideSourceBookOriginal('item', name) || hideSourceBookOriginal('spell', name)) {
        saveToBackend();
        return;
    }
    const list = getDeletedItems();
    const normalized = normalizeEntryName(name);
    if (!list.some((d) => namesMatch(d, normalized))) {
...
function deleteItemPermanently(name, type) {
    if (type === 'monster') {
        ...
        const overrideMonsters = getOverrideMonsters().filter(m => m.name !== name);
        localStorage.setItem('dnd_extension_custom_monsters', JSON.stringify(customMonsters));
        localStorage.setItem('dnd_extension_override_monsters', JSON.stringify(overrideMonsters));
    } else if (type === 'item') {
        const customItems = getCustomItems().filter(i => i.name !== name);
        const overrideItems = getOverrideItems().filter(i => i.name !== name);
        localStorage.setItem('dnd_extension_custom_items', JSON.stringify(customItems));
        localStorage.setItem('dnd_extension_override_items', JSON.stringify(overrideItems));
    } else if (type === 'spell') {
        const customSpells = getCustomSpells().filter(s => s.name !== name);
        const overrideSpells = getOverrideSpells().filter(s => s.name !== name);
        localStorage.setItem('dnd_extension_custom_spells', JSON.stringify(customSpells));
        localStorage.setItem('dnd_extension_override_spells', JSON.stringify(overrideSpells));
    }
...
export function searchItems(query) {
  const deleted = getDeletedItems();
  const overrides = getOverrideItems();
  const activeItems = mergeSourceBookEntries(items, overrides, deleted);
...
// Helper: Spawn Moveable AoE Template
              ...
              await OBR.scene.items.updateItems(extensionItems.map(i => i.id), (items) => {
                  for (let item of items) {
                      ...
                      if (isMonster && (!cr || cr === "Unknown")) {
                          const allMonsters = [...monsters, ...getOverrideMonsters(), ...getCustomMonsters()];
  ...
  const openEditor = (mode, data = null) => {
      ...
      if (data) {
          ...
          const override = getOverrideList(type).find((entry) => namesMatch(entry.name, data.name));
          if (builtIn) {
      ...
      if (editorOriginalName && editorOriginalName !== name && !sourceBookRenameHandled) {
          if (editorMode === 'monster') {
              ...
              if (oldIdx !== -1) {
                  ...
                  const overrides = getOverrideMonsters();
                  const overrideIdx = overrides.findIndex(m => m.name === editorOriginalName);
                  if (overrideIdx !== -1) {
                      if (confirm(`Do you want to remove your edits to "${editorOriginalName}"?`)) {
                          overrides.splice(overrideIdx, 1);
                          localStorage.setItem('dnd_extension_override_monsters', JSON.stringify(overrides));
                          saveToBackend();
                      }
                  } else if (confirm(`Do you want to hide the original entry "${editorOriginalName}"?`)) {
              ...
              if (oldIdx !== -1) {
                  ...
                  const overrides = getOverrideSpells();
                  const overrideIdx = overrides.findIndex(s => s.name === editorOriginalName);
                  if (overrideIdx !== -1) {
                      if (confirm(`Do you want to remove your edits to "${editorOriginalName}"?`)) {
                          overrides.splice(overrideIdx, 1);
                          localStorage.setItem('dnd_extension_override_spells', JSON.stringify(overrides));
                          saveToBackend();
                      }
                  } else if (confirm(`Do you want to hide the original entry "${editorOriginalName}"?`)) {
                      deleteItem(editorOriginalName);
              ...
              if (oldIdx !== -1) {
                  ...
                  const overrides = getOverrideItems();
                  const overrideIdx = overrides.findIndex(i => i.name === editorOriginalName);
                  if (overrideIdx !== -1) {
                      if (confirm(`Do you want to remove your edits to "${editorOriginalName}"?`)) {
                          overrides.splice(overrideIdx, 1);
                          localStorage.setItem('dnd_extension_override_items', JSON.stringify(overrides));
                          saveToBackend();
                      }
                  } else if (confirm(`Do you want to hide the original entry "${editorOriginalName}"?`)) {
                      deleteItem(editorOriginalName);
    ...
    pdfUploadInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        try {
          ...
          localMonsters = aiResult.monsters || [];
          localItems = aiResult.items || [];
          
          localMonsters = localMonsters.map(m => ({
            rawText: m.rawText,
            parsed: {
              name: m.name,
              hp: m.hp,
          ...
          localMonsters = extractMonstersFromPDFText(text);
          localItems = extractItemsFromPDFText(text);
        }
        
        extractedMonsters = localMonsters;
        extractedItems = localItems;
  ...
  if (exportSourceBtn) {
      exportSourceBtn.addEventListener('click', () => {
          ...
          const customs = getCustomMonsters();
          const overrides = getOverrideMonsters();
          const finalMonsters = [...mergeSourceBookEntries(monsters, overrides, deleted), ...customs].map(m => {
          ...
          const itemOverrides = getOverrideItems();
          const finalItems = [...mergeSourceBookEntries(items, itemOverrides, deleted), ...customItems].map(i => {
  ...
  if (randomBtn) {
      randomBtn.addEventListener('click', () => {
          const deleted = getDeletedItems();
          const allItems = [
              ...mergeSourceBookEntries(items, getOverrideItems(), deleted),
              ...getCustomItems().filter(i => !deleted.includes(i.name)),
          ];
    ...
    if (e.target && e.target.id === 'save-new-image') {
      ...
      const saveAndApply = async (imgSrc, isRetry = false) => {
        try {
          ...
          // Always try to remove background when saving/applying a new image!
          try {
            console.log('Removing background before saving/applying image...');
            imgSrc = await processAndRemoveBackground(imgSrc);
          } catch (bgError) {
            console.warn('Background removal failed, using original:', bgError);
          }
  ...
  const showStats = (data, itemId) => {
    ...
    if (isSpell) {
        libraryData = getSpellByName(data.name);
        if (libraryData) {
            descriptionToUse = libraryData.description;
            data = { ...data, ...libraryData };
        }
    } else if (isItem) {
        libraryData = getItemByName(data.name);
        if (libraryData) {
            descriptionToUse = libraryData.description;
            data = { ...data, ...libraryData };
        }
    } else {
        libraryData = getMonsterByName(data.name);
        if (libraryData) {
            descriptionToUse = libraryData.description;
            data = { ...data, ...libraryData };
        }
    ...
    if (saveDescBtn) {
      saveDescBtn.addEventListener('click', async () => {
        ...
        if (isMonster) {
          saved = saveMonsterEntry({ ...data, description: newDesc }, { originalName: data.name });
        } else if (isItem) {
          saved = saveItemEntry({ ...data, description: newDesc }, { originalName: data.name });
        } else if (isSpell) {
          saved = saveSpellEntry({ ...data, description: newDesc }, { originalName: data.name });
        }
    ...
    if (!isSpell && !isItem) {
      ...
      if (refreshBtn) {
        refreshBtn.addEventListener('click', async () => {
          ...
          const overrides = getOverrideMonsters();
          const customs = getCustomMonsters();
          let idx = overrides.findIndex(m => m.name === data.name);
          let monster = idx !== -1 ? overrides[idx] : null;
          if (!monster) {
            idx = customs.findIndex(m => m.name === data.name);
            monster = idx !== -1 ? customs[idx] : null;
          }
          if (!monster) {
            const builtIn = monsters.find(m => m.name === data.name);
            if (!builtIn) {
              alert("Could not find monster data to refresh.");
              return;
            }
            monster = { ...builtIn };
            overrides.push(monster);
          ...
          if (updated) {
            saveMonsterEntry(monster, { originalName: data.name });
            ...
            const updatedData = getMonsterByName(data.name);
    ...
    msInput.addEventListener('input', (e) => {
        ...
        const overrideNames = getOverrideSpells().map(s => s.name);
        const customNames = getCustomSpells().map(s => s.name);
        const builtInNames = Object.keys(SPELL_DATA);
        const allNames = [...new Set([...overrideNames, ...customNames, ...builtInNames])];
        ...
        const updateTokenStats = async (newHp, newAc) => {
            try {
                ...
                const updatedMonster = { 
                    ...
                    hp: newHp, 
                    ac: newAc,
                    source: data.source || getMonsterByName(data.name)?.source || "Custom" 
                };
                
                saveMonsterEntry(updatedMonster, { originalName: data.name });
            ...
            hpResetBtn.addEventListener('click', async () => {
                try {
                    ...
                    if (items.length > 0) {
                        ...
                        const original = getMonsterByName(monsterName);
                        if (original) {
  ...
  const renderResults = (query) => {
    ...
    if (activeTab === 'monsters' && isGM) {
        ...
        const customMonsters = getCustomMonsters().filter((m) => !isBuiltInMonster(m.name) && !m.originBookName);
        const customItems = getCustomItems().filter((i) => !isBuiltInItem(i.name) && !i.originBookName);
        const customSpells = getCustomSpells().filter((s) => !isBuiltInSpell(s.name) && !s.originBookName);
